...
const SingleEntryPlugin = require('webpack/lib/SingleEntryPlugin'); // eslint-disable-line
...
module.exports.compileTemplate = function compileTemplate(options, context, compilation) {
  ...
  childCompiler.apply(new SingleEntryPlugin(context, `!!${require.resolve('./favicons.js')}?${
    JSON.stringify({
      ...options,
    })}!${options.source}`));
  ...
  childCompiler.plugin('compilation', (compilation) => { // eslint-disable-line
    if (compilation.cache) {
      if (!compilation.cache[compilerName]) {
        compilation.cache[compilerName] = {}; // eslint-disable-line
      }
      compilation.cache = compilation.cache[compilerName]; // eslint-disable-line
    }
    compilation.plugin('optimize-chunk-assets', (chunks, callback) => {
      ...
      compilation.assets[resultFile] = { // eslint-disable-line
        source() {
          return resultJson;
        },
        size() {
